...
init() ->
    ...
    receive
        ...
        Other->
            ?ERR(init, "unknown message ~p", [Other])
    end.
...
parse_login(Socket, Client) ->
    ...
    receive
        {inet_async, Socket, Ref, {ok, <<TotleLen:16, Cmd:16, _IsCompressed:8>>}} ->
            ?INFO(receive_cmd,"receive cmd ~p",[Cmd]),
            ...
            case BodyLen >= 0 of
                ...
                false ->
                    ?ERR(parse_login, "Error! this pack has no body")
            end;
...
parse(Socket, Client) ->
    ...
    receive
        {inet_async, Socket, Ref, {ok, <<TotleLen:16, Cmd:16, _IsCompressed:8>>}} ->
            ?INFO(parse, "%%%%%recv a new Cmd [~p] with totle length:~p%%%%", [Cmd, TotleLen]),
            BodyLen = TotleLen - ?HEADER_LENGTH,
            case BodyLen >= 0 of
                true ->
                    handle_parse_recv(Socket, BodyLen, Client, Cmd);
                false ->
                    ?ERR(parse, "Error! body < 0 ?")
            end;
        {inet_async, Socket, Ref, {error, timeout}} ->
            case Client#client.timeout_count >= ?HEART_TIMEOUT_MAX_COUNT of
                true ->
                    close_conn(Socket);
                false ->
                    ?INFO(parse, "timeout for tcp receiving, (~p) times~n", [Client#client.timeout_count]),
                    parse(Socket, Client#client{timeout_count = Client#client.timeout_count + 1})
            end;
        {inet_async, Socket, Ref, {error, etimedout}} ->
            ?INFO(parse, "error, etimedout"),
            close_conn(Socket);
        {'EXIT', From, Reason} ->
            ?ERR(parse, "EXIT!, From:~p, Reason:~p", [From, Reason]);
        {inet_async, Socket, Ref, {error, closed}} ->
            ok;
        Other ->
            ?INFO(parse, "Reason:~p", [Other])
    end.
...
handle_no_body(Socket, Client, Cmd) ->
    case catch gen_server:call(Client#client.player_pid, {routing, Cmd, <<>>}) of
        ...
        error -> 
            ?ERR(parse, "player process down!continue to parse!"),
            parse(Socket, Client#client{timeout_count = 0});
        {'EXIT', Reason} ->
            ?ERR(parse, "Error! Reason:~p", [Reason])
    end.
        ...
        {inet_async, Socket, RefData, {ok, Bin}} ->
            case extract(Cmd, Bin) of 
                ...
                {bad_arg, Arg} -> %% 协议格式ok，但数据不正常，忽略，但不断开连接
                    ?WARNING(parse, "Bad arg:~p", [Arg]),
                    parse(Socket, Client#client{timeout_count = 0});
                ignore -> %% 协议格式ok，但协议数据不符合或该用户无权限执行GM指令，忽略，但不断开连接
                    ?WARNING(parse, "No Privilege! Ignore GM CMD from:~p, User is:~p", 
                ...
                Other -> %% 协议格式异常，认为是非正常客户端，断开连接
                    ?ERR(parse, "Error! Disconnect Client! ~nReason:~p", [Other])
            end;
        Other ->
            ?ERR(parse, "Error! Reason:~p", [Other])
    end.
...
%%关闭连接(仅超时和下线时关闭)
close_conn(Socket) ->
    ?INFO(close_conn, "Timeout! Close TCP!"),
